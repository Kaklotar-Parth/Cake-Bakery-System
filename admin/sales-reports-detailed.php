...
<?php
    ...
    sum(tblorders.Price) as totalitmprice from tblorders
...
<?php
    $ret = mysqli_query($con, "select year(OrderTime) as lyear,sum(tblorders.Price) as totalitmprice from tblorders 
    join tblorderaddresses on tblorderaddresses.Ordernumber=tblorders.OrderNumber 
    where year(tblorderaddresses.OrderTime) between '$fdate' and '$tdate' 
    and tblorderaddresses.OrderFinalStatus='$fstatus' group by lyear");
            $cnt = 1;
            while ($row = mysqli_fetch_array($ret)) {
...
<?php }?>
            ...
            <div class="text-center mt-4 mb-3">
                        <a  style="background-color: #0B5ED7; color:white;"  href="sales-reports.php" class="btn btn-secondary">
                            <i class="fa fa-arrow-left"></i> Back to sale reports
                        </a>
                    </div>
                </div>
